...
#' #Suppose the current clustering membership indicates 3 patients in cluster 1, 2 patients in cluster 2, 3 patinets in cluster 3, 1 patient in cluster 4
...
#' @seealso \link{update_RJ} for a complete example for all functions in this package.
#' @examples  
#' #Suppose we know the number of patient level cluster is 2, one has 2 site level clusters and one has 3.
#' #Use the default value, 1, for hyper-parameter
#' update_w(K=c(2,3),R=matrix(c(1,2,2,1,1,2,3,2),nrow=2,byrow=T), S=2)
#' 
#' #To change the hyper-parameter to, for example 2
#' update_w(K=c(2,3),R=matrix(c(1,2,2,1,1,2,3,2),nrow=2,byrow=T), S=2, hyper_delta = 2)
#' @details 
...
#' @param c The linear coefficients for 
...
#'   }
#' @export
update_RJ <- function(w, K, Gamma,Beta, Z, X, R, mu, mu_star, Y, delta, c, sigma_square, C,
                      S, theta,q,
                      hyper_delta = 1){
  for(i in 1:S){
    wi<- w[i, 1:K[i]]
...
likeli.theta<-function(theta,tau, Beta){
...
#' @param Beta The linear coefficients for patient level covariates. Should be a matrix with S rows
#' @param sig The hyper-parameter (sd) with default value being 10
#' @return updated DPP hyper-parameter for patient level
#' @seealso \link{update_RJ} for a complete example for all functions in this package.
#' @export
update.theta.beta<-function(theta, tau, Beta,sig = 10){
  theta.new<- theta + (rnorm(1,0,0.1))
  ratio <- (likeli.theta(theta.new,tau,Beta) + dnorm(theta.new,0,sig,log=TRUE) -
              likeli.theta(theta,tau,Beta) - dnorm(theta,0,sig,log=TRUE))
...
#' @seealso \link{update_RJ} for a complete example for all functions in this package.
#' @export
update.theta.gamma<-function(theta,tau,Gamma, S, Ds, sig = 10 ){
  theta.new<- theta + (rnorm(1,0,0.1))
  ratio <- 0
  for(i in 1:S){
    Gammatemp<-Gamma[1:Ds[i],,i]
    ratio <- ratio + (likeli.theta(theta.new,tau,Gammatemp) + dnorm(theta.new,0,sig,log=TRUE) -
                        likeli.theta(theta,tau,Gammatemp) - dnorm(theta,0,sig,log=TRUE))
  }
  if(ratio>log(runif(1))){
    return(theta.new)
  }
